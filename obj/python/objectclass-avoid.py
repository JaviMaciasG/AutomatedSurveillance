...
class person(thing):
    ...
    def DetermineNewOrUpdate(posx,posy,width,height,people,framespassed,img,histogramvalue,updatetype,kalmanarray):
...
class backpack(thing):
    ...
    def DetermineNewOrUpdate(posx, posy, width, height, bags,framespassed,people,img,histogramvalue,updatetype,KalmanArray):
        xcentroid = round(posx + (width - posx)/2)
        ycentroid = round(posy + (height - posy)/2)
        #cv2.circle(img, (xcentroid, ycentroid), 5, (0, 0, 255), -1)
        Isnew = 0
        ...
        if (changed2 != 1) and (EuDist(xcentroid, ycentroid, bags[0].xcentroid, bags[0].ycentroid) > 5):
            ...
            if updatetype == 0:
                newkalmanfilter = createkalman(xcentroid,ycentroid)
                newkalman = associatedkalman(bags[0].id,"bags",newkalmanfilter)
                KalmanArray.append(newkalman)

            return (bags[0].id),KalmanArray
        ...
        for x in range(Array_length):
            ...
            if updatetype == 0:
                if KalmanArray[x].name == "bag":
                    KalmanX = KalmanArray[x].kalmanfilter.X[0]
                    KalmanY = KalmanArray[x].kalmanfilter.X[1]
                    minlistval = EuDist(xcentroid, ycentroid, KalmanX, KalmanY)
                    print(minlistval, " with bag", KalmanArray[x].id)

                    objectposition = CheckPosition(bags,KalmanArray[x].id)
                    KalmanW = bags[objectposition].width
                    KalmanH = bags[objectposition].height

                    difference, greater = AreaDiff(width, height, KalmanW, KalmanH, posx, posy, (KalmanX-(KalmanW - KalmanX)/2),
                                                   (KalmanY - (KalmanH - KalmanY) / 2), 1)
                    minDiff.append(difference)
                else:
                    continue
        ...
        if updatetype == 0:
            ...
            Kalmanminpos = np.argmin(themin)
            theminpos = CheckPosition(bags,KalmanArray[Kalmanminpos].id)
        ...
        if (themin < mean/3.5) and (bags[theminpos].updated == 0):
            Isnew = 0
        ...
        if Isnew == 0:
            ...
            bags[theminpos].updatevalue(posx, posy, width, height, framespassed,histogramvalue)
            print("Bag ", bags[theminpos].id, " updated")
            return(bags[theminpos].id),KalmanArray
