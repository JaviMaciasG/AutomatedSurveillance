...
def createMegaBlocks(motionInfoOfFrames,noOfRows,noOfCols):
    ...
    n = 2
    megaBlockMotInfVal = np.zeros((round(noOfRows/n),round(noOfCols/n),len(motionInfoOfFrames),8))
    ...
    for frame in motionInfoOfFrames:
        ...
        for index,val in np.ndenumerate(frame[...,0]):
            ...
            megaBlockMotInfVal[int(index[0]/n)][int(index[1]/n)][frameCounter] = np.array(map(sum, zip(*temp)))
